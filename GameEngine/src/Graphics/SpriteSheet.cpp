...
{
	bool loadedImage = mBMPImage.Load(App::Singleton().GetAssetsPath() + name + ".bmp");
	bool loadedSpriteSections = LoadSpriteSections(App::Singleton().GetAssetsPath() + name + ".txt");
...
Sprite SpriteSheet::GetSpriteAt(const uint32_t& index) const
{
	return mSections[index].sprite;
}
