...
SDL_Window* Screen::Init(uint32_t w, uint32_t h, uint32_t mag, bool fast)
{
	...
	moptrWindow = SDL_CreateWindow("Arcade", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, mWidth * mag, mHeight * mag, SDL_WINDOW_FOREIGN);
...
{
	Line2D l0 = { star.GetPointAtIndex(0), star.GetPointAtIndex(2) };
	Line2D l1 = { star.GetPointAtIndex(1), star.GetPointAtIndex(3) };
	Line2D l2 = { star.GetPointAtIndex(2), star.GetPointAtIndex(4) };
	Line2D l3 = { star.GetPointAtIndex(3), star.GetPointAtIndex(0) };
	Line2D l4 = { star.GetPointAtIndex(4), star.GetPointAtIndex(1) };
...
void Screen::Draw(const Triangle& triangle, const Color& color, bool fill, const Color& fillColor)
{
	...
	Line2D l1 = { triangle.GetP0(), triangle.GetP1() };
	Line2D l2 = { triangle.GetP1(), triangle.GetP2() };
	Line2D l3 = { triangle.GetP2(), triangle.GetP0() };
...
{
	if (fill) FillPoly(rectangle.GetPoints(), [fillColor](uint32_t x, uint32_t y) {return fillColor; });

	std::vector<Vec2D> points = rectangle.GetPoints();

	Line2D l1 = { points[0], points[1] };
	Line2D l2 = { points[1], points[2] };
	Line2D l3 = { points[2], points[3] };
	Line2D l4 = { points[3], points[0] };
...
void Screen::Draw(const BMPImage& image, const Sprite& sprite, const Vec2D& pos, const Color& overlayColor)
{
	...
	const float inYAxisLengthSq = 1.0f / yAxis.Mag2();
	...
	FillPoly(points, [&](uint32_t px, uint32_t py)
		{
			...
			float v = inYAxisLengthSq * d.Dot(yAxis);
...
void Screen::FillPoly(const std::vector<Vec2D>& points, FillPolyFunc function)
{
	if (points.size() > 0)
	{
		...
		for (int pixelY = top; pixelY < bottom; pixelY++)
		{
			...
			for (size_t k = 0; k < nodeXVec.size(); k += 2)
			{
				...
				if (nodeXVec[k + 1] > left)
				{
					...
					{
						Draw(pixelX, pixelY, function(pixelX, pixelY));
					}
				}
			}
		}
