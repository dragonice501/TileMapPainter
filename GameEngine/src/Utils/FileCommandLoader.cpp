...
{
	size_t nextSpacePos = params.line.find_first_of(" ", params.dimlimitPos + 1);
	int width = std::stoi(params.line.substr(params.dimlimitPos, nextSpacePos - params.dimlimitPos));
	size_t lastSpacePos = nextSpacePos;
	nextSpacePos = params.line.find_first_of(" ", lastSpacePos + 1);
...
{
	std::string intStr = params.line.substr(params.dimlimitPos + 1);
	return std::stoi(intStr);
}

std::string FileCommandLoader::ReadString(const ParseFuncParams& params)
{
	return params.line.substr(params.dimlimitPos + 1);
}

char FileCommandLoader::ReadChar(const ParseFuncParams& params)
{
	return params.line.substr(params.dimlimitPos + 1)[0];
}
