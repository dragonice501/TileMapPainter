...
{
	if (mSelectionRectEnd.GetY() == mMapHeight - 1) return;
	...
	for (uint16_t y = mSelectionRectStart.GetY() + mSelectionHeight - 1; y >= mSelectionRectStart.GetY(); y--)
	{
		for (uint16_t x = mSelectionRectStart.GetX() + mSelectionWidth - 1; x >= mSelectionRectStart.GetX(); x--)
		{
			mMapSpriteIndeces[x][y + 1] = mMapSpriteIndeces[x][y];
		}
	}
...
{
	if (mSelectionRectEnd.GetX() == mMapWidth - 1) return;
