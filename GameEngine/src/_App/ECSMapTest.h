...
class ECSMapTest : public ECSScene
{
public:
	ECSMapTest();

	virtual void Init(std::unique_ptr<Registry>& registry, std::unique_ptr<AssetStore>& assetStore, SDL_Renderer* renderer) override;
	virtual void Destroy() override;
	virtual void Setup(std::unique_ptr<Registry>& registry) override;

	virtual void Input(std::unique_ptr<Registry>& registry) override;
	virtual void Update(std::unique_ptr<Registry>& registry, uint32_t deltaTime) override;
	virtual void Render(std::unique_ptr<Registry>& registry, std::unique_ptr<AssetStore>& assetStore, SDL_Renderer* renderer) override;

	virtual const bool IsRunning() const override { return mIsRunning; }
